...
- **TWO-OFFER CONVERSION LIVE (D26, founder-approved "Yes, deploy now"):** site now sells exactly two offers — **Sofrito Session ($400, 90 min, written action-plan)** and **Brand & Web Sprint ($997)**. Session $400 up front books the spot and is credited toward a Sprint booked within 30 days; Sprint balance $597 settles before launch; files release just after the balance clears. Worker **Version `141e0390-3769-4b5a-8d79-77310d180520`** (15 files). Invoice engine: `SESSION_CENTS=40000`, milestones `session`→`final`, split = `{session: min(price,40000), final: price-session}` → 40000/59700.
...
- ## In flight (no sign-off required — reversible, housekeeping)
  - **Two-offer conversion DEPLOYED (D26):** version `141e0390-3769-4b5a-8d79-77310d180520` live (15 files). Decision logged; `now.md` + sprint-intake figures aligned.
