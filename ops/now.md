...
> Snapshot 2026-09-12. This file is the single 5-minute catch-up for anyone (founder or agent) resuming work. When something changes, update it — do not let it go stale.

## Right now (2026-09-12)
- **LOGO PLACEMENT OPEN (D27, founder request 2026-09-12):** founder uploaded \Sofritostudio.com_Trans-logo.jpg\ and wants it on the site **header, footer, and package cards**. Model **cannot read** the uploaded image (no image input), so the exact file was never visually confirmed. Existing brand assets on disk: \pivot-site\public\assets\brand\\\ (\logo.svg\, \logo-badge-full.svg\, \logo-badge-full.png\, \logo-badge-enhanced.png\, \logo-badge-text.png\, \logo-master.jpeg\), plus \pivot-site\public\assets\logo.svg\ / \logo-master.jpeg\. **FOUNDER ACTION: confirm which asset/path to use (or save the uploaded logo to disk and give the path).** Editing header/footer/package-card markup is staged-only until founder approves the exact asset + a fresh deploy (AGENTS.md deploy gate - prior approvals do not carry over).
- **TWO-OFFER CONVERSION LIVE (D26, founder-approved "Yes, deploy now"):** site now sells exactly two offers — **Sofrito Session ($400, 90 min, written action-plan)** and **Brand & Web Sprint ($997)**. Session $400 up front books the spot and is credited toward a Sprint booked within 30 days; Sprint balance $597 settles before launch; files release just after the balance clears. Worker **Version `5e4ea067-ac8c-41bd-ab3a-35d65fd90b23`** (15 files). Invoice engine: `SESSION_CENTS=40000`, milestones `session`→`final`, split = `{session: min(price,40000), final: price-session}` → 40000/59700.
- **Calendly event created (D23):** "Sofrito Strategy Session", 60 min, active, `https://calendly.com/j-ortiz1148/sofrito-strategy-session`. **`is_paid: false` — the $400 gate is NOT live yet; founder must enable payment (Stripe/Square) in Calendly UI. Note: Calendly event is configured at $300 — site now promises $400 (D26 model); founder should set the gate to $400.**
...
- **Two-offer conversion DEPLOYED (D26):** version `5e4ea067-...` live (15 files). Decision logged; `now.md` + sprint-intake figures aligned.
